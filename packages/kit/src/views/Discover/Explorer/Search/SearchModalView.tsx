...
export const SearchModalView: FC = () => {
  ...
  const flatListData = useMemo(() => {
    ...
    let _flatListData = searchContentTerm ? searchedDapps : allHistories;
    _flatListData = _flatListData.map((item) => {
      if (item.dapp) {
        item.dapp.url = item.dapp.url.replace(/onekey/gi, '');
        item.dapp.name = item.dapp.name.replace(/onekey/gi, '');
      }
      return item;
    });
    return _flatListData;
