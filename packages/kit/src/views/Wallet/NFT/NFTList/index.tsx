...
import NFTlistObjData from '@onekeyhq/kit/src/utils/data/NFTlistObjData';
import { hashToName } from '@onekeyhq/kit/src/utils/data/contractAddress';
...
function NFTListContainer() {
  ...
  const getOwnerAllToken = useCallback(() => {
    if (account?.address && networkId === 'evm--9000') {
      axios({
        method: 'get',
        url: "https://explorer.depaas.de/exapi/api/nft/getOwnerAllToken",
        params: {
          contract: '',
        ...
        // console.log(res)
        if (res.status === 200) {
          const listData = res.data.content.map(item => {
            let _NFTlistObjData = JSON.parse(JSON.stringify(NFTlistObjData))
            ...
            _NFTlistObjData = {
              ...
              priceSymbol: hashToName[item.contract] || 'DE'
            }

            _NFTlistObjData.assets = [{
              ..._NFTlistObjData.assets[0],
              contractTokenId: item.tokenId,
              tokenId: item.tokenId,
              owner: item.owner,
              name: item.name,
              latestTradePrice: item.price,
              nftscanUri: item.picture,
              collection: {
                logoUrl: item.picture,
              },
            }]

            return _NFTlistObjData
          })
          useCollectiblesDE(listData)
        }
      })
    }
  }, [account, networkId])
  ...
  const fetchData = async () => {
    ...
    if (account && networkId && networkId !== 'evm--9000') {
  ...
  const [collectiblesDE, useCollectiblesDE] = useState<Collection[]>([]);

  // useEffect(() => {
  // getOwnerAllToken();
  // }, [account, networkId, getOwnerAllToken])
  ...
  return (
    <NFTListContentProvider>
      <NFTList
        collectibles={networkId === 'evm--9000' ? collectiblesDE : collectibles}
        onSelectCollection={handleSelectCollectible}
        onSelectAsset={handleSelectAsset}
        fetchData={mutate}
        isNFTSupport={networkId === 'evm--9000' ? true : isNFTSupport}
        isLoading={isLoading}
      />
    </NFTListContentProvider>
  );
