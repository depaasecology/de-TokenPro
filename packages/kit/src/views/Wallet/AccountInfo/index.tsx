...
import { Dimensions, Platform, Linking, TouchableOpacity } from 'react-native';
...
export const FIXED_VERTICAL_HEADER_HEIGHT = 238 + 140;
export const FIXED_HORIZONTAL_HEDER_HEIGHT = 152 + 140;

export interface HomeCarouselItem {
  'id': number;
  'imgUrlCn': string;
  'imgUrlEn': string;
  'relationType': 2 | 3 | 4 | 5;
  'relationHref': string;
...
const AccountOption: FC<AccountOptionProps> = ({ isSmallView }) => {
  ...
  const { wallet, account, network, networkId } = useActiveWalletAccount();
...
const AccountInfo = ({
  ...
  imgUri?: HomeCarouselItem[];
  setCloseRefresh?: unknown;
}) => {
  const intl = useIntl();
  const isSmallView = useIsVerticalLayout();
  const openLinkingFn = useCallback(async (item: HomeCarouselItem) => {
    if (item.relationType === 3) {
      return;
    }
    if (item.relationType === 2) {
      openUrl(item.relationHref);
      return;
    }
    if (item.relationType === 5) {
      let url: string | null = '';
      if (Platform.OS === 'ios') {
        url = item.iosLink;
      } else {
        url = item.androidLink;
      }
  ...
  const [showSwiper, setsShowSwiper] = useState(true);
  useFocusEffect(
    useCallback(() => {
      if (Platform.OS === 'web') {
        setsShowSwiper(false);
        setTimeout(() => {
          setsShowSwiper(true);
        }, 10);
      }
  ...
  if (isSmallView) {
    return (
      ...
      >
        {showSwiper && imgUri?.length === 0 ? (
          <CustomSkeleton w="100%" h="140px" mb="16px" borderRadius="xl" />
        ) : (
          <Box
            alignItems="center"
            w="100%"
          ...
          >
            ...
            >
              {imgUri?.map((item: HomeCarouselItem) => {
                try {
                  const imgUrl = intl.locale.includes('zh')
                    ? item.imgUrlCn
                    : item.imgUrlEn;
                  return (
                    <Pressable
