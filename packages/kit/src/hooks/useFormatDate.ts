...
export default function useFormatDate() {
  ...
  const format = useCallback(
    (date: Date | string, _format?: string, _localeShow?: 'zh-CN') => {
      let parsedDate: Date;
      ...
      if (_localeShow === 'zh-CN' && _localeShow !== locale) {
        _format = _format?.replace(/年|日/g, '');

        if (/y.*M.*d.*H.*m.*s/.test(String(_format))) {
          _format = 'PPpp';
        }
        if (/y.*M.*d.*H.*m/.test(String(_format))) {
          _format = 'PPp';
        }
        if (/y.*M.*d/.test(String(_format))) {
          _format = 'PP';
        }
      }
      try {
